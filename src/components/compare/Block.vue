<template>
  <div>
    <h4 class="overline mb-6 mt-8 ml-4">{{ block_title }}</h4>
     <div class="mx-sm-6 mx-2 compareBlockHeader" v-if="!isFieldOfStudy && config && config.medianToggle && config.type == 'average-annual-cost'" ><p class="median-value-text" >National {{config.medianToggle === 'group' ? block_title.replace('schools', 'Midpoint') : "Midpoint"}}: {{$options.filters.numeral(config.medianToggle === 'group' ? fakeAverageAnnualCost[_.get(schools[0], fields['PREDOMINANT_DEGREE'])][0] : fakeAverageAnnualCost[_.get(schools[0], fields['PREDOMINANT_DEGREE'])][1] ,'$0,0')}}</p></div>
