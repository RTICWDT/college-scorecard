<template>
  <div>
    <h4 class="overline mb-6 mt-8 ml-4">{{ block_title }}</h4>
     <div class="mx-sm-6 mx-2 compareBlockHeader" v-if="config && config.medianToggle && config.type == 'average-annual-cost'" ><p class="median-value-text" >National {{config.medianToggle === 'group' ? block_title.replace('schools', 'Comparison') : "Comparison"}}: {{$options.filters.numeral(config.medianToggle === 'group' ? fakeAverageAnnualCost[parseInt(groupName)] : fakeAverageAnnualCost[4] ,'$0,0')}}</p></div>
     <div class="mx-sm-6 mx-2 compareBlockHeader" v-if="config && config.medianToggle && config.type == 'median-earnings'" ><p class="median-value-text" >National {{config.medianToggle === 'group' ? block_title.replace('schools', 'Comparison') : "Comparison"}}: {{$options.filters.numeral(config.medianToggle === 'group' ? fakeMedianEarnings[parseInt(groupName)] : fakeMedianEarnings[4] ,'$0,0')}}</p></div>
     <div class="mx-sm-6 mx-2 compareBlockHeader" v-if="config && config.medianToggle && config.type == 'graduation-rate'" ><p class="median-value-text" >National {{config.medianToggle === 'group' ? block_title.replace('schools', 'Comparison') : "Comparison"}}: {{$options.filters.numeral(config.medianToggle === 'group' ? fakeGraduationRate[parseInt(groupName)] : fakeGraduationRate[4] ,'0%')}}</p></div>  
